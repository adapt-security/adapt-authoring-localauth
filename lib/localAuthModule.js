...
class LocalAuthModule extends AbstractModule {
  ...
  */
  async init() {
    ...
    */
    ...
    auth.unsecureRoute(`${this.router.path}/forgotpassword`, 'post');
    auth.unsecureRoute(`${this.router.path}/resetpassword`, 'post');
  ...
  * @param {String|Object} userIdOrQuery Accepts a user _id or a query object
  * @param {Object} updateData JSON data to use for update
  * @return {Promise}
  */
  async updateUser(userIdOrQuery, updateData) {
    const query = _.isString(userIdOrQuery) ? { _id: userIdOrQuery } : userIdOrQuery;
    return this.users.update(query, await this.validate(updateData));
  }
  /**
  ...
  */
  async forgotPasswordHandler(req, res, next) {
    ...
    try {
      ...
      const text = `Please visit the following link in your web browser to reset your password:\n${url}`;
      const html = `<h2>Reset password</h2><p>Someone (hopefully you) has requested to reset the password for your Adapt account.<a href="${url}">Click this link to reset your Adapt password</a></p><p>If you didn't request to change your password, you can safely ignore this email and the link will expire shortly.</p>`;
