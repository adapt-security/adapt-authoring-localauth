...
class LocalAuthModule extends AbstractModule {
  ...
  */
  ...
  async applyLocking(user) {
    ...
    await this.users.update({ _id: user._id }, { failedAttempts, isTempLocked, isPermLocked, lastFailedLoginAttempt });
  ...
  */
  async register(data) {
    ...
    return this.app.auth.findOrCreateUser('local', data.email, { ...data, password });
  }
  /**
