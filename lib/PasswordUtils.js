...
class PasswordUtils {
  ...
  static async compare(plainPassword, hash) {
    if(!plainPassword || !hash) {
      throw App.instance.errors.INVALID_PARAMS.setData({ params: ['plainPassword', 'hash'] });
    }
    try {
      const isValid = await promisify(bcrypt.compare)(plainPassword, hash);
      if(!isValid) throw new Error();
    } catch(e) {
      throw App.instance.errors.INCORRECT_PASSWORD;
    }
  }
  /**
  ...
  static async validate(password) {
    ...
    const match = (key, re) => !localauth.getConfig(key) || password.search(re) > -1;
    const validationChecks = {
      INVALID_PASSWORD_LENGTH: [password.length >= localauth.getConfig('minPasswordLength'), { length: localauth.getConfig('minPasswordLength') }],
