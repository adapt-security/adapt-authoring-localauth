...
class LocalAuthModule extends AbstractAuthModule {
  ...
  async init() {
    ...
    users.preUpdateHook.tap(async (oldData, newData) => { // we don't allow passwords to be set using the user API
      if(newData.password) throw this.app.errors.API_PASSWORD_CHANGE_FORBIDDEN;
    });
    users.sanitiseDataHook.tap(data => {
  ...
  async authenticate(req, res) {
    ...
    try {
      ...
      this.log('debug', 'FAILED_LOGIN', !user ? 'INVALID_USER' : user?._id?.toString());
      throw this.app.errors.UNAUTHENTICATED;
    }
  }
  /**
  ...
  async unlockHandler(req, res, next) {
    try {
      ...
      this.log('debug', 'USER_UNLOCK', req.params._id, req?.auth?.user?._id?.toString());
      res.status(204).end();
    } catch(e) {
