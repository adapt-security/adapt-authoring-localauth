...
class LocalAuthModule extends AbstractAuthModule {
  ...
  async validatePassword(password) {
    ...
    const match = (key, re) => !this.getConfig(key) || password.search(re) > -1;
    const validationChecks = {
      INVALID_PASSWORD_LENGTH: [password.length >= this.getConfig('minPasswordLength'), { length: this.getConfig('minPasswordLength') }],
    ...
    const errors = Object.entries(validationChecks).reduce((m, [code, [isValid, data]]) => {
      if(!isValid) m.push(this.app.errors[code].setData(data));
      return m;
    }, []);
    if(errors.length) throw this.app.errors.INVALID_PASSWORD.setData({ errors });
  }
  /**
  ...
  async registerSuperHandler(req, res, next) {
    try {
      const [roles, users] = await this.waitForModule('roles', 'users');
      const [superRole] = await roles.find({ shortName: 'superuser' });
      const superUsers = users.find({ roles: [superRole._id] });
      if(superUsers.length) {
        throw this.app.errors.SUPER_USER_EXISTS;
      }
      await this.register({
        email: req.body.email, 
        password: req.bod.passwordy,
        firstName: 'Super',
        lastName: 'User',
  ...
  async validatePasswordHandler(req, res, next) {
    try {
      await this.validatePassword(req.body.password);
      res.json({ message: req.translate('app.passwordindicatorstrong') });
    } catch(e) {
      e.data.errors = e.data.errors.map(e => req.translate(`error.${e.code}`, e.data)).join(', ');
      res.sendError(e);
    }
  }
}
