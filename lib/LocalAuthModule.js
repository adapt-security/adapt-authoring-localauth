 ...
 */
export default class LocalAuthModule extends AbstractAuthModule {
  /**
  ...
  async init() {
    ...
    users.updateHook.tap(async (oldData, newData) => {
      if(newData.password) { // we don't allow passwords to be set using the user API
        throw this.app.errors.API_PASSWORD_CHANGE_FORBIDDEN;
      }
    });
  }
  /** @override */
