...
class LocalAuthModule extends AbstractAuthModule {
    ...
    /** @ignore */ this.routes = [
      {
        ...
        route: '/changepass',
        handlers: { get: this.changePasswordHandler.bind(this), post: this.changePasswordHandler.bind(this) }
      }, {
        route: '/forgotpass',
  ...
  async init() {
    ...
    this.unsecureRoute(`/changepass`, 'get');
    ...
    users.updateHook.tap(async (oldData, newData, opts) => {
      if(newData.password) {
        /**
  ...
  async updateUser(userIdOrQuery, updateData) {
    ...
    const data = await this.validate(updateData, { useDefaults: false, ignoreRequired: true });
    const user = await this.users.update(query, data);
   ...
   */
  async applyLocking(userData) {
    const failedLoginAttempts = userData.failedLoginAttempts;
    ...
    return { ...userData, isTempLocked, isPermLocked };
  }
  /**
   ...
   * @return {Promise}
   */
  async validate(dataToValidate, options = { ignoreRequired: true }) {
    const jsonschema = await this.app.waitForModule('jsonschema');
    return jsonschema.validate('localauthuser', dataToValidate, options);
  }
  /**
  ...
  async changePasswordHandler(req, res, next) {
    let email;
    try {
      if(req.method === 'GET') { // unauthenticated requests should use get (i.e. users coming from an email link)
        await PasswordUtils.validateReset(req.body.email, req.body.token);
        await PasswordUtils.deleteReset(req.body.token);
        email = req.body.email;
      } else if(req.method === 'POST') { // all authenticated API requests should use POST
